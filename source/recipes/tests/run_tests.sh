...
# Clean up any potentially problematic containers and start fresh
echo "Cleaning up Docker environment..."
cd ../../../
docker-compose down -v || true  # Force down even if there's an error
docker system prune -f  # Clean up any dangling images/containers

# Start the Docker Compose environment for testing
echo "Starting Docker Compose environment..."
docker-compose pull || true  # Try pulling images first
docker-compose build  # Build fresh images
docker-compose up -d backend login main nginx shared-templates recipes
...
if [ "$SKIP_HEALTH_CHECK" = false ]; then
  ...
  if ! check_service_health "recipes" "http://localhost:8007/health"; then
      echo "Failed to start recipes service, stopping test execution."
      [ "$KEEP_RUNNING" = false ] && docker-compose down
      exit 1
  fi
...
# Run the recipes tests
echo "Running recipes integration tests..."
cd source/recipes/tests

if [ -n "$SPECIFIC_TEST" ]; then
  echo "Running specific test: $SPECIFIC_TEST"
  PYTHONPATH=../.. pytest -v test_recipes.py::TestRecipesIntegration::$SPECIFIC_TEST
else
  PYTHONPATH=../.. pytest -v test_recipes.py
fi
...
echo "  --test=test_name: Run only a specific test (e.g. --test=test_recipes_page_access_technician)"
echo ""
echo "Example: ./run_tests.sh --skip-health-check --keep-running --test=test_recipes_page_access_manager"
echo ""
