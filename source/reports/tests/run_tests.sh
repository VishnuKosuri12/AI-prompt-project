...
docker-compose up -d backend login main nginx shared-templates reports
...
if [ "$SKIP_HEALTH_CHECK" = false ]; then
  ...
  if ! check_service_health "reports" "http://localhost:8008/health"; then
      echo "Failed to start reports service, stopping test execution."
      [ "$KEEP_RUNNING" = false ] && docker-compose down
      exit 1
  fi
...
# Run the reports tests
echo "Running reports integration tests..."
cd source/reports/tests

if [ -n "$SPECIFIC_TEST" ]; then
  echo "Running specific test: $SPECIFIC_TEST"
  PYTHONPATH=../.. pytest -v test_reports.py::TestReportsIntegration::$SPECIFIC_TEST
else
  PYTHONPATH=../.. pytest -v test_reports.py
fi
...
echo "  --test=test_name: Run only a specific test (e.g. --test=test_reports_page_access_technician)"
echo ""
echo "Example: ./run_tests.sh --skip-health-check --keep-running --test=test_reports_page_without_login"
echo ""
